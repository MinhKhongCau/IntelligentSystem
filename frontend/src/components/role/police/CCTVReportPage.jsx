...
const CCTVReportPage = () => {
  const location = useLocation();
  const navigate = useNavigate();
  const { detection, detectionIndex, totalDetections, searchResults } = location.state || {};

  useEffect(() => {
    // Redirect if no detection data
    if (!detection) {
      navigate('/police/cctv-monitor');
    }
  }, [detection, navigate]);
  ...
  const handleBack = () => {
    navigate('/police/cctv-monitor', { state: { searchResults } });
  };

  const handlePrevious = () => {
    if (detectionIndex > 0 && searchResults?.detections) {
      const prevDetection = searchResults.detections[detectionIndex - 1];
  ...
  const handleNext = () => {
    if (detectionIndex < totalDetections - 1 && searchResults?.detections) {
      const nextDetection = searchResults.detections[detectionIndex + 1];
      navigate('/police/cctv-report', {
        state: {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 p-6">
      <div className="max-w-6xl mx-auto">
        {/* Header */}
        <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-xl shadow-2xl p-6 mb-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                disabled={detectionIndex === 0}
                className="px-4 py-2 bg-white bg-opacity-20 hover:bg-opacity-30 rounded-lg font-semibold transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
              >
              ...
              <button
                onClick={handleNext}
                disabled={detectionIndex === totalDetections - 1}
                className="px-4 py-2 bg-white bg-opacity-20 hover:bg-opacity-30 rounded-lg font-semibold transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
              >
                Next
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="bg-white rounded-xl shadow-2xl p-6 space-y-6">
          ...
          <div className="flex gap-3 pt-4 border-t">
            <button
              onClick={handleBack}
              className="flex-1 px-6 py-3 bg-gray-200 hover:bg-gray-300 text-gray-700 rounded-lg font-semibold transition-all flex items-center justify-center gap-2"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" />
              </svg>
